...
export class StreamableHTTPTransport implements MCPTransport {
	...
	private readonly headersOverride: HeadersInit;
	...
	private async establishReceiveStream(): Promise<void> {
		if (this.eventSource || this.isClosed) {
			console.log(
				`[GET Stream] Skipping establish: eventSource=${!!this
					.eventSource}, isClosed=${this.isClosed}`,
			);
			...
			if (contentType?.includes('application/json')) {
				...
				for (const msg of messages) {
					...
					if (typeof msg === 'object' && msg !== null && 'jsonrpc' in msg) {
						...
						this.handleError(
							new MCPClientError({
								message: `Received invalid JSON message structure: ${JSON.stringify(
									msg,
								)}`,
							}),
						);
