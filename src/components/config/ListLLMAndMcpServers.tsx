					...
					config.llms.map((llm: LLM, index: number) => (
						...
						>
							...
							<Box marginLeft={2} flexDirection="column">
								{llm.id && (
									<Text>
										ID: <Text color="gray">{llm.id}</Text>
									</Text>
								)}
								{llm.apiUrl && (
									<Text>
										API URL: <Text color="gray">{llm.apiUrl}</Text>
									</Text>
								)}
								{llm.apiVersion && (
									<Text>
										API Version: <Text color="gray">{llm.apiVersion}</Text>
									</Text>
								)}
							</Box>
						</Box>
			...
			<Box flexDirection="column">
				...
				{!config.mcpServers || Object.keys(config.mcpServers).length === 0 ? (
					...
					Object.entries(config.mcpServers).map(
						([key, server]: [string, MCPServerConfig], index: number) => (
							<Box
								key={index}
								flexDirection="column"
								marginLeft={2}
							...
							>
								<Text bold color="green">
									{key}: <Text color="white">{server.name}</Text>
								</Text>
								<Box marginLeft={2} flexDirection="column">
									<Text>
									...
									{server.transport === 'stdio' && (
										<>
											<Text>
												Command: <Text color="gray">{server.command}</Text>
											</Text>
											<Text>
												Args: <Text color="gray">{server.args.join(' ')}</Text>
											</Text>
										</>
									)}
									{server.transport !== 'stdio' && 'url' in server && (
										<Text>
											URL: <Text color="gray">{server.url}</Text>
										</Text>
									)}
									{server.version && (
										<Text>
											Version: <Text color="gray">{server.version}</Text>
										</Text>
									)}
								</Box>
							</Box>
